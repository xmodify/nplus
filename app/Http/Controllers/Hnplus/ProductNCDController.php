...
//ncd_report--------------------------------------------------------------------------------------------------------------------------
    public function ncd_report(Request $request)
    {
        ...
        $product_summary=DB::select('
            ...
            (SUM(patient_all)*(SUM(patient_hr)/SUM(patient_all))*(1.4/8))/COUNT(shift_time) AS nurse_shift_time
...
//ncd_morning_save------------------------------------------------------------------------------------------------------------------
    public function ncd_morning_save(Request $request)
    {
        ...
        $nurse_hr    = $nurse_total * 8;  // NCD ใช้ 9 ชั่วโมง
        $productivity = ($patient_hr * 100) / max(1, $nurse_hr);
        $hhpuos = $patient_hr / max(1, $patient_all);
        $nurse_shift_time = $patient_all * $hhpuos * (1.4 / 8);
