...
use App\Models\Productivity_psy;
use App\Models\MainSetting;
use Illuminate\Routing\Controllers\Middleware;

#[Middleware('auth', only: ['psy_report', 'psy_product_delete'])]

class ProductPSYController extends Controller
{
    //psy_report--------------------------------------------------------------------------------------------------------------------------
    public function psy_report(Request $request)
    {
        ...
        $product = Productivity_psy::whereBetween('report_date', [$start_date, $end_date])
        ...
        $product_summary = DB::select('
            ...
            FROM productivity_psy
            WHERE report_date BETWEEN ? AND ?
            GROUP BY shift_time ORDER BY shift_time DESC', [$psy_working_hours, $start_date, $end_date]);

        // เตรียมข้อมูลสำหรับกราฟ
        $product_asc = Productivity_psy::whereBetween('report_date', [$start_date, $end_date])
        ...
        return view('hnplus.product.psy_report', compact(
            'product_summary',
            'product',
            'start_date',
            'end_date',
    ...
    //product_delete----------------------------------------------------------------------------------------------------------------
    public function psy_product_delete($id)
    {
        $product = Productivity_psy::find($id)->delete();
        return redirect()->route('hnplus.product.psy_report')->with('danger', 'ลบข้อมูลเรียบร้อยแล้ว');
    }

    //แจ้งเตือนสถานะการณ์สรุปเวรเช้า รัน 16.00 น.---------------------------------------------------------------------------------------------
    public function psy_morning_notify()
    {
        ...
        $notify = DB::connection('hosxp')->select("
            ...
            FROM opd_dep_queue o1, ovst o2 WHERE o1.depcode IN ($psy_dep)
            AND o1.vn = o2.vn AND o2.vstdate = DATE(NOW())
            AND o2.vsttime BETWEEN '00:00:00' AND '15:59:59' ");

        foreach ($notify as $row) {
            $patient_all = $row->patient_all;
            $url = url('product/psy_morning');
        }

        //แจ้งเตือน Telegram
        $message = "🧑‍⚕️งานสุขภาพจิตและยาเสพติด" . "\n"
        ...
        $chat_ids = explode(',', MainSetting::where('name', 'psy_notifytelegram')->value('value'));
    ...
    public function psy_morning()
    {
        ...
        return view('hnplus.product.psy_morning', compact('shift'));
    }

    //psy_morning_save------------------------------------------------------------------------------------------------------------------
    public function psy_morning_save(Request $request)
    {
        ...
        $nurse_shift_time = $patient_all * $nhppd * (1.4 / $psy_working_hours);

        Productivity_psy::updateOrCreate(
            [
        ...
        $token = MainSetting::where('name', 'telegram_token')->value('value');
        $chat_ids = explode(',', MainSetting::where('name', 'psy_notifytelegram_save')->value('value'));
