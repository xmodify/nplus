...
Route::prefix('admin')->middleware(['auth', 'is_admin'])->name('admin.')->group(function () {
    ...
    Route::get('main_setting/download_ps1/{filename}', function ($filename) {
        $allowed = [
            'Run_Tasks_Product_SummaryNight',
            'Run_Tasks_Product_SummaryMorning',
            'Run_Tasks_Product_SummaryAfternoon',
            'Run_Tasks_Product_SummaryBd',
        ];
        if (!in_array($filename, $allowed)) {
            abort(404);
        }
        $path = base_path('manual/' . $filename . '.ps1');
        if (!file_exists($path)) {
            abort(404);
        }
        return response()->download($path, $filename . '.ps1', [
...
Route::middleware(['auth', 'hnplus'])->name('hnplus.')->group(function () {
    ...
    Route::match(['get', 'post'], 'product/icu_report', [ProductICUController::class, 'icu_report'])->name('product.icu_report');
    Route::delete('product/icu_product_delete/{id}', [ProductICUController::class, 'icu_product_delete']);
...
Route::name('hnplus.')->group(function () {
    ...
    //product ICU-----------------------------------------------------------------------------------------------------------
    Route::get('product/icu_night_notify', [ProductICUController::class, 'icu_night_notify']);
    Route::get('product/icu_night', [ProductICUController::class, 'icu_night']);
    Route::post('product/icu_night_save', [ProductICUController::class, 'icu_night_save']);
    Route::get('product/icu_morning_notify', [ProductICUController::class, 'icu_morning_notify']);
    Route::get('product/icu_morning', [ProductICUController::class, 'icu_morning']);
    Route::post('product/icu_morning_save', [ProductICUController::class, 'icu_morning_save']);
    Route::get('product/icu_afternoon_notify', [ProductICUController::class, 'icu_afternoon_notify']);
    Route::get('product/icu_afternoon', [ProductICUController::class, 'icu_afternoon']);
    Route::post('product/icu_afternoon_save', [ProductICUController::class, 'icu_afternoon_save']);
    //product LR-----------------------------------------------------------------------------------------------------------
